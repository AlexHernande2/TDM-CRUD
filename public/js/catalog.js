...
function renderItem(item) {
  ...
  card.innerHTML = `
        <img src="${item.img || "https://picsum.photos/300/200"}" alt="${
    item.name
  }">
        <h3>${item.name}</h3>
  ...
  card.querySelector(".details-btn").addEventListener("click", async () => {
    try {
      // Hacer petición al endpoint de detalle
      const res = await fetch(`${API_URL}/${item.id}`);
      const fullItem = await res.json();

      // Llenar modal con los datos completos
      modalName.textContent = fullItem.name;
